...
public sealed class DeathRecorder
{
    ...
    private readonly record struct Pending(DateTime Time, RecapKind Kind, uint SourceId, uint AbilityKey, long Amount,
        long Overheal, bool Crit, bool DirectHit, bool Parried, bool Blocked, TargetHp? Hp);
    ...
    public void Damage(uint victimId, DateTime time, uint sourceId, uint abilityKey, long amount, bool crit, bool directHit,
        bool parried, bool blocked, TargetHp? hp) =>
        Add(victimId, new Pending(time, RecapKind.Damage, sourceId, abilityKey, amount, 0, crit, directHit, parried, blocked, hp));
    ...
    public void Heal(uint targetId, DateTime time, uint sourceId, uint abilityKey, long amount, long overheal, bool crit, TargetHp? hp) =>
        Add(targetId, new Pending(time, RecapKind.Heal, sourceId, abilityKey, amount, overheal, crit, false, false, false, hp));

    public void Miss(uint targetId, DateTime time, uint sourceId, uint abilityKey) =>
        Add(targetId, new Pending(time, RecapKind.Miss, sourceId, abilityKey, 0, 0, false, false, false, false, null));
        ...
        if (recent.TryGetValue(victimId, out var queue))
        {
            foreach (var e in queue.Where(e => time - e.Time <= Window).Reverse())
            {
                ...
                events.Add(new RecapEvent((time - e.Time).TotalSeconds, e.Kind, nameOf(e.SourceId), e.AbilityKey, e.Amount,
                    e.Overheal, e.Crit, e.DirectHit, e.Parried, e.Blocked, hpAfter, e.Hp?.Max ?? 0));
            }
            queue.Clear();
        }
